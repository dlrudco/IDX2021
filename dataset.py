...
class RespiDataset_Diag(Dataset):
    def __init__(self, split, initialize=True, data_dir="", num_mel=128, multi_label=False, mean=None, std=None, fixed_length=None):
        ...
        if os.path.isfile(os.path.join(data_dir,'../diagnosis.txt')) and os.path.isfile(os.path.join(data_dir,'../cls2id.txt')):
            diags = open(os.path.join(data_dir,'../diagnosis.txt'), 'r').read()
            self.diagdict = {}
            for pat in diags.split('\n'):
                pat_id, disease = pat.split(',')
                self.diagdict[pat_id] = disease
            self.cls2idx = {}
            clss = open(os.path.join(data_dir,'../cls2id.txt'), 'r').read()
            self.idx2cls = ["None"]*len(clss.split('\n'))
            for cl in clss.split('\n'):
                dis, idx = cl.split(',')
                self.cls2idx[dis] = int(idx)
                self.idx2cls[int(idx)] = dis
        else:
